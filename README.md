...
**`tagarr_import_migrate.sh`** — Migrates an existing `tagarr_import.conf` to the latest format. Merges your current values (API keys, release groups, webhooks, filters) into the new sample template, adding any new settings with safe defaults. Never modifies your original config — writes to `tagarr_import.conf.new` for review. Run after updating to a new version that adds config options.
